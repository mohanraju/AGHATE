...
function updateForms(VID,Name,Id,Val,Source,ABV,libelle,compdata) {
		...
		var ColorActuel=$("#LBL_"+Name+Id).attr('class');
		var valeur=Val;
		if(Name=='observ' || Name=='afaire' || Name=='atcd')valeur=$('#'+Name).val();
...
function returnView(file,id,TableLoc){
	var variables="filename="+file;
	res=LanceAjax("../commun/ajax/retourVue.php",variables);
	if (res=="KO")alert("Erreur id veuillez reessayer plus tard..");
	if(res.length>0){
		window.location ="reservation_aghate.php?id="+id+"&file="+file+"&table_loc="+TableLoc+"&name=LBL_forms"+res;
	}
	else{
		window.location ="reservation_aghate.php?id=38&file=Reservation"+"&table_loc="+TableLoc;
	}   		
}
//=================================================================
// document ready
