 ...
 * #####################################################################
 * modie le 02/07/2014
 * prise en comptes les NOIP de ORBIS dans le etape 0
 * 
 * 
 * 
*/
...
/*=======================================================================
 * Etape 0 
 * Mettre a jour des nip pour orbis
 * ======================================================================
*/

$Aghate->AddTrace("\n ####  MAJ NIP ORBIS ####\n"); 	
//$jour_debut= $Aghate->MakeDate("01/01/2014","00:01");
$jour_debut= $Aghate->MakeDate(date('d/m/Y'),"00:01");
//echo $jour_debut."<br>";
$sql="SELECT * from agt_loc where noip not like '800%' and start_time > ".$jour_debut."";
...
for($i =$start ; $i < $nbr; $i++)
{
	$sql="SELECT * from pat where noip='".$res[$i]['noip']."'";
	//print_r ($res[$i]);
	//echo "<br>".$sql;
	$ResPat=$Gilda->OraSelect($sql);
	echo "<br>nip:".$res[$i]['noip']."  nouveau nip".$ResPat[0]['NOIPFU'];
	if ((count($ResPat) > 0) && (strlen($ResPat[0]['NOIPFU']) > 9) )
	{

		$sql_updt=$Mysql->execute("update agt_loc set noip='".$ResPat[0]['NOIPFU']."' where noip='".$res[$i]['noip']."'");
		echo "<br>loc:".$sql_updt;	

		//check patient exixte dejé dans le table agt_pat
		$res=$Mysql->select("select * from  agt_pat where noip='".$ResPat[0]['NOIPFU']."'");
		if (count($res) < 1)
		{
			$sql_updt_pat=$Mysql->execute("update agt_pat set noip='".$ResPat[0]['NOIPFU']."' where noip='".$res[$i]['noip']."'");
			echo "<br>pat:".$sql_updt_pat;	
		}	
		else
		{
			echo "<br>pat:".$ResPat[0]['NOIPFU']." existe déja";	
		}	
			
		$sql_updt_loc_backup=$Mysql->execute("update loc_backup set NOIP='".$ResPat[0]['NOIPFU']."' where NOIP='".$res[$i]['noip']."'");
		echo "<br>loc_backup:".$sql_updt_loc_backup;	

		$sql_updt_prog=$Mysql->execute("update agt_prog set noip='".$ResPat[0]['NOIPFU']."' where noip='".$res[$i]['noip']."'");	
		echo "<br>prog:".$sql_updt_prog;	
	}
	$Aghate->AddTrace("\n ancien nip:".$res[$i]['noip']."  nouveau nip:".$ResPat[0]['NOIPFU']."");
	...
	for($s=0 ; $s < count($sejours); $s++)
	{
		...
		elseif(($sejours[$s]['start_time']	== $date_deb) && ($sejours[$s]['room_id']	== $room_id) && ($sejours[$s]['ds_source']=="Programme"))		
		{
			if ( $sejours[$s]['end_time'] < time())
			{
				...
				// check lit occupé par autre patient, donv envoi le NIP a exclure
				$ChkPlaceLibre=$Aghate->IsPlaceLibre($room_id,$date_deb,time(),$nip);
				// si place libre
				if(count($ChkPlaceLibre) > 0)
				{
					//deplace patient qui occupe au couloir
