...
//    $midnight2 = gmmktime(0, 0, 0, $month_num, $day_num, $year_num);
    ...
    while ($t < $end_t)
    {
        ...
        if (strlen($row[$i]['noip'])>0)   
			...
			$patient="(".$row[$i]['patient'].")";				
        $d[$day_num]["who1"][] = $patient;
		...
		if (strlen($row[$i]['medecin'])>0){
			...
			$desc_medecin="<br>Medecin : ". $med['nom']. " ". $med['prenom'];
 		}else
			$desc_medecin="";

		$protocole=(strlen($row[$i]['protocole'])>0)?"<br>protocole : ".$row[$i]['protocole']:"";

        $d[$day_num]["description"][] = $protocole.$desc_medecin." <br>".$Aghate->GetDescComplementaire($row[$i]['description']) ;
...
$room_info = $Aghate->GetRoomsByServiceId($area,$Panier=ture);
            ...
            for ($i = 0; $i < $n; $i++) {
                ...
                if ($d[$cday]["id_room"][$i]==$row[$ir]['id']) {
                    ...
                    echo "\n<table width='100%' border='0'><tr>";
						...
						$UrlView=$ModuleReservationView."?id=".$d[$cday]["id"][$i]."&table_loc=".$Aghate->NomTableLoc;
						echo "<font size=-2><b>". $d[$cday]["data"][$i]
						. "<br /></b>"
						."<a href='#?'  onClick=\"OpenPopupResa('".$UrlView."')\"   title=\"".$title."\" >"					
