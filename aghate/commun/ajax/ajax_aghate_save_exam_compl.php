...
if(strlen($patient)>0) {
 ...
 //----------------------------------------------------------------------------- 
// check medecin present dans la table agt_medecin_exam_compl
//-----------------------------------------------------------------------------
 
if (strlen($id_medecin) < 1)
{
	if(strlen($medecin)<1){
		echo "|ERR| Medecin obligatoire";
		exit;
	}
	else{
		$sql = "INSERT INTO ".$table_medecin." SET nom='".$medecin."'";
		$id_medecin = $mysql->insert($sql);
	}
}
...
//si date_deb existe
if(strlen($date_deb)>0){
	list($day,$month,$year)=explode("/",$date_deb);
}
...
if(strlen($room_id)<1){
	echo "|ERR|Lit non renseigné ";
	exit;
}

$id_service = $Aghate->GetServiceIdByRoomId($room_id);

//check plage occupé par autre patient // cas multi user
$Chk=$Aghate->IsPlageLibre($plage_pos,date('d/m/Y',$starttime),$room_id);


if(count($Chk) > 0){
	
	//on cherche le premier place dispo dans le service
	$Plages=$Aghate->GetServicePlages($id_service);

	for($p=0; $p < count($Plages); $p++)
	{
		$Chk=$Aghate->IsPlageLibre($Plages[$p]['num_periode'],date('d/m/Y',$starttime),$room_id);
		// new plage pos
		if(count($Chk) < 1){
			$plage_pos	=$Plages[$p]['num_periode'];
			break;
		}
