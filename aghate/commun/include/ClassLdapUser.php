...
class LdapUser extends MySQL{
	...
	function grr_opensession($_login, $_password, $_user_ext_authentifie = '', $tab_login=array(), $tab_groups=array()){
		...
		if (($num_row > 0) and isset($_SESSION['start'])) {
			$sql = "update agt_log set END = now() + interval " . getSettingValue("sessionMaxLength") . 
					" minute where SESSION_ID = '" . session_id() . "' and START = '" . $_SESSION['start'] . "'";
	...
	//      session_destroy();
		...
		if (($row['default_area'] !='') and ($row['default_area'] !='0')) $_SESSION['default_area'] = $row['default_area']; else $_SESSION['default_area'] = getSettingValue("default_area");
		if (($row['default_room'] !='') and ($row['default_room'] !='0')) $_SESSION['default_room'] = $row['default_room']; else $_SESSION['default_room'] = getSettingValue("default_room");
		if ($row['default_style'] !='') $_SESSION['default_style'] = $row['default_style']; else $_SESSION['default_style'] = getSettingValue("default_css");
		if ($row['default_list_type'] !='') $_SESSION['default_list_type'] = $row['default_list_type']; else $_SESSION['default_list_type'] = getSettingValue("area_list_format");
		if ($row['default_language'] !='') $_SESSION['default_language'] = $row['default_language']; else $_SESSION['default_language'] = getSettingValue("default_language");
