...
?>
<script type="text/javascript" src="./commun/js/functions.js"></script>
<?php
...
//  if ($fieldtype == "list") $obligatoire = "n";
  ...
  if ( $arearight == True)
    {
      $sql = "insert into agt_overload (id_area, fieldname, fieldtype, obligatoire, confidentiel, fieldlist, affichage, overload_mail) values ($id_area, '".protect_data_sql($fieldname)."', '".protect_data_sql($fieldtype)."', '".$obligatoire."', '".$confidentiel."', '".protect_data_sql($fieldlist)."', '".$affichage."', '".$overload_mail."');";
...
if ($action == "delete" )
{
  ...
  $sql = "select id_area from agt_overload where id=$id_overload;";
...
foreach( $userdomain as $key=>$value )
{
  $res = grr_sql_query("select id, fieldname, fieldtype, obligatoire, fieldlist, affichage, overload_mail, confidentiel from agt_overload where id_area=$key order by fieldname;");
  if (! $res) fatal_error(0, grr_sql_error());
