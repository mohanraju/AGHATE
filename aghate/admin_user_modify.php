...
if ($valid == "yes") {
    ...
    $droit_demande = isset($_GET["droit_demande"]) ? $_GET["droit_demande"] : NULL;    
...
//
        ...
        if ($retry != 'yes') {
            ...
            etat='".protect_data_sql($reg_etat)."',
            medecin='".protect_data_sql($medecin)."',
            droit_demande='".protect_data_sql($droit_demande)."'                        
...
if (isset($user_login) and ($user_login!='')) {
    $sql = "SELECT nom, prenom, statut, etat, email, source,medecin,droit_demande FROM agt_utilisateurs WHERE login='$user_login'";
    $res = grr_sql_query($sql);
    if ($res) {
...
echo "<tr>
		<td>Droit d'effectuer une demande:</td>
		<td><input type=\"radio\" name=\"droit_demande\" value=\"1\" "; 
			if ($droit_demande == "1") { echo "checked";}
			echo " >OUI
		<input type=\"radio\" name=\"droit_demande\" value=\"0\" "; if ($droit_demande == "0") { echo "checked";} echo ">NON
		</td>
	</tr>";
echo "</table>";
