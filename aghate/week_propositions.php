...
$sql = "SELECT start_time, end_time, agt_loc.id, name, beneficiaire, agt_room.id,type, statut_entry, agt_loc.description, agt_loc.option_reservation, agt_room.delais_option_reservation, agt_loc.moderate, beneficiaire_ext
...
if (grr_sql_count($res) == 0)
{
  ...
  for ($ir = 0; ($row = grr_sql_row($res, $ir)); $ir++)
    ...
    echo "<tr>\n";
    echo tdcell("cell_hours","5")."<a title=\"".htmlspecialchars(get_vocab("see_week_for_this_room"))."\" href='week.php?year=".$year."&amp;month=".$month."&amp;day=".$day."&amp;room=".$row[2]."'>" . htmlspecialchars($row[0]) ."</a><br />\n";
    if (verif_display_fiche_ressource(getUserName(), $row[2]) and $_GET['pview'] != 1)
  	...
  	if (authGetUserLevel(getUserName(),$row[2]) > 2 and $_GET['pview'] != 1)
        echo "<a href='admin_edit_room.php?room=$row[2]'><img src=\"./commun/images/editor.png\" alt=\"configuration\" border=\"0\" title=\"".get_vocab("Configurer la ressource")."\" width=\"30\" height=\"30\" class=\"print_image\"  /></a></td>";
        ...
        if ((isset($d[$cday]["id"][0])) and  !(est_hors_reservation(mktime(0,0,0,$cmonth,$cday,$cyear)))) {
            ...
            for ($i = 0; $i < $n; $i++) {
                ...
                if ($d[$cday]["id_room"][$i]==$row[2]) {
                    ...
                    if ((isset($d[$cday]["option_reser"][$i])) and ($d[$cday]["option_reser"][$i]!=-1)) echo "&nbsp;<img src=\"./commun/images/small_flag.png\" alt=\"".get_vocab("reservation_a_confirmer_au_plus_tard_le")."\" title=\"".get_vocab("reservation_a_confirmer_au_plus_tard_le")."&nbsp;".time_date_string_jma($d[$cday]["option_reser"][$i],$dformat)."\" width=\"20\" height=\"20\" border=\"0\" />&nbsp;\n";
        ...
        if (est_hors_reservation(mktime(0,0,0,$cmonth,$cday,$cyear)))
            ...
            and (($row[4] == "1") or
              (($row[4] == "0") and (authGetUserLevel(getUserName(),$room) > 2) ))) {
                if ($enable_periods == 'y')
                    echo "<a href=\"edit_entry.php?area=$area&amp;room=".$row[2]."&amp;period=&amp;year=$cyear&amp;month=$cmonth&amp;day=$cday&amp;page=week_all\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"./commun/images/new.png\" border=\"0\" alt=\"".get_vocab("add")."\" class=\"print_image\"  /></a>";
                else
                    echo "<a href=\"edit_entry.php?area=$area&amp;room=".$row[2]."&amp;hour=$hour&amp;minute=0&amp;year=$cyear&amp;month=$cmonth&amp;day=$cday&amp;page=week_all\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"./commun/images/new.png\" border=\"0\" alt=\"".get_vocab("add")."\" class=\"print_image\"  /></a>";
            } else {
                echo "&nbsp;";
            }
        echo "</center>";
