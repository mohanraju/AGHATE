...
{
	var oTable =$('#ProtocoleList').dataTable( {
	"sScrollY": 300,
	"bJQueryUI": true,
	"iDisplayLength": 10,	
	"bPaginate": false,							
...
function popup_protocole(id_protocole){
	service_id=$("#service_id").val();
	OpenPopup("./protocoles_add.php?id_protocole="+id_protocole+"&service_id="+service_id)
} 
...
$sql="select * from agt_protocole where service='".$service_id."' order by actif desc, protocole";
...
<form name="UserInfo" id="UserInfo" method="POST" action="">	
	<div id="body_left">
		 ...
		 ?>
			...
			<input type="button" name="ajout" value="Ajouter" onclick="popup_protocole('NEW')" 	class="btn btn-success" >  	 		
			<table cellpadding="0" cellspacing="0" border="0" class="display" id="ProtocoleList" >
				<thead>
					<tr>
						<th>Protocole</th>
						<th>Durée</th>
				...
				for($i=0; $i < count($results);$i++) {
					...
					print 
					"<tr>
						<td>".$results[$i]['protocole']."</td>
						<td>".$results[$i]['duree']."</td>
						<td>".$etat."</td>
						<td><a href='#' onclick='popup_protocole(".$results[$i]['id_protocole'].")'><img src='./commun/images/edit.jpg' border='0' height='15px'></a></td>
					</tr>	
					";
				}
				?> 
