...
$service_ipop = isset($_POST["service_ipop"]) ? $_POST["service_ipop"] : NULL;
$service_type = isset($_POST["service_type"]) ? $_POST["service_type"] : NULL;
$nom_formulaire = isset($_POST["nom_formulaire"]) ? $_POST["nom_formulaire"] : NULL;
...
//echo $sql;            
  ...
  if ($access=='a')
    {
        $sql = "DELETE FROM agt_j_user_area WHERE id_area='$area'";
        if (grr_sql_command($sql) < 0)
            fatal_error(0, get_vocab('update_area_failed') . grr_sql_error());
    }
    if ((isset($change_done)) and (!isset($ok))) {
 ...
 else
   {
    ...
    echo "<TD>".$Html->InputSelect($ListeTypes,'service_type',$row["service_type"],100)."</TD>\n";
    echo "</TR>";    

	// libelle service ipop pour faire le mapping avec ipop/aghate
    echo "<TD>Libelle Service IPOP </TD>\n";
    echo "<TD>".$Html->InputTextBox('service_ipop',htmlspecialchars($row["service_ipop"]),100,20)."</TD>\n";
    echo "</TR>";    

	// libelle service ipop pour faire le mapping avec ipop/aghate
    echo "<TD>Nom Formulaire  </TD>\n";
    echo "<TD>".$Html->InputTextBox('nom_formulaire',htmlspecialchars($row["nom_formulaire"]),100,20)."</TD>\n";
    echo "</TR>";   
