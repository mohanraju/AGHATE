...
if (isset($id))
{
    $sql = "select name, beneficiaire, description, start_time, end_time,
            type, room_id, entry_type, repeat_id, option_reservation, jours, create_by, beneficiaire_ext ,hds,noip,medecin from agt_loc where id=$id";
...
function validate_and_submit ()
{
   ...
   //------------------------------
	...
	val= jour +'-'+mois+'-'+annee+'|'+noip+'|'+parseInt(c_id);   
  ...
  if (document.forms["main"].benef_ext_nom) {
	...
	s=document.forms["main"].type.value;
	ss=document.forms["main"].sexe_attend.value;
	if (sexe_attendu != "I"){
		if(document.forms["main"].type.value != sexe_attendu){
...
//echo "<select name=\"areas\" onChange=\"changeRooms(this.form)\" >";
  echo "<select id=\"areas\" name=\"areas\" onChange=\"changeRooms(this.form);insertChampsAdd();insertTypes()\" readonly>";
...
echo "<TR><td class=\"CL\" valign=\"top\"><table border=0><tr><td><select name=\"rooms[]\" id=\"c_rooms\">";
...
echo "</div></td></tr>";
//mohan ajouter les type de reservation HDJ ou HDS
	$hds_oui="";
	$hdj_oui="checked=\"checked\"";	
	
if ($hds=="HDS"){
	$hds_oui="checked=\"checked\"";	
	$hdj_oui="";	
	}
echo "<tr><td class='E'><b> Type d'hôpitalisation".get_vocab('deux_points')."</b>";

echo "  <input type=\"radio\" name=\"hds\"   value=\"HDJ\"  ".$hdj_oui." />
			<img src=\"./commun/images/hdj.gif\" width=\"35\" height=\"20\"/> 
  			<input type=\"radio\" name=\"hds\"    value=\"HDS\" ".$hds_oui. " />
  			<img src=\"./commun/images/hds.gif\" width=\"40\" height=\"23\"/>  			";

echo "</TD></TR>\n";
// selection de médecin
echo "<tr><td class='E'><b> Médecin".get_vocab('deux_points')."</b><select name=\"medecin\" id=\"medecin\">";
$sql = "select titre,nom,prenom from agt_medecin order by nom";
$res = grr_sql_query($sql);
if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
{
  $selected = "";

  if ($row[1] == $med) $selected = "SELECTED";
  echo "<option $selected value=\"".$row[1]."\">".$row[1];
}
