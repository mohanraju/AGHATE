...
===================================================================
*/
function UpdateTimes(obj_id)
{
	var cpt = 1 ;// new compteuur pos
	...
	$('#'+obj_id+' tr').each(function() {
		...
		LocId =$(this).attr( "LOC_ID" ); 		
	 	...
	 	{
			$(this).find('td').eq(0).html(cpt);
			
			// update pos dans la base si pat present dans le td
			if (LocId)
			{

				 LanceAjax("./commun/ajax/ajax_aghate_resa_update_pos.php","id="+LocId+"&newpos="+cpt+"&table_loc="+TableLoc);		 
			}
			cpt++;
		}
	});	
...
static $ecolors;
	...
	for($r = 0; $r < count($Rooms); $r++)
	{
		...
		$JS_data .= '$( "#'.$Rooms[$r]['room_alias'].'" ).sortable({
					  ...
					  helper:"clone",
					  zIndex: 999990,
					  update: function( event, ui ) {UpdateTimes(\''.$Rooms[$r]['room_alias'].'\')},
				  })
				  .disableSelection() ;';
		
		
		echo "<td align='center'> <H4>".$Rooms[$r]['room_alias']."</H4>";
		echo "<table id='TBL_".$Rooms[$r]['room_alias']."' cellspacing='0' cellpadding='0' border='1'>";
		...
		for($plage=0; $plage < $NbrPlages; $plage++)
		{
			...
			for($p=0; $p < $nb_res; $p++)
			{
				if (($res[$p]['plage_pos']==$plages[$plage]['num_periode']) &&  $res[$p]['room_id']==$Rooms[$r]['id'] )
				{
					...
					// donc force sur le prochange... a voir
		...
		for($p=0;$p < count($TableauHeure);$p++)
		{
			if (!is_array($TableauHeure[$p]['data']))
			{
				...
				$UrlEdit=$ModuleReservationEdit."?area=".$area."&room=".$Rooms[$r]['id']."&hour=".$hour."&minute=".$minute."&year=".$year."&month=".$month."&day=".$day."&page=day&plage_pos=".$TableauHeure[$p]['pos']."&table_loc=".$TableLoc;
			...
			}else
			{
				...
				$UrlEdit=$ModuleReservationView."?id=".$TableauHeure[$p]['data']['entry_id']."&table_loc=".$TableLoc;
				$lien= "<a href='#?'  onClick=\"OpenPopupResa('".$UrlEdit."')\">"
						.$imgag_link. " "
						."NIP : ".$TableauHeure[$p]['data']['noip']." / "
		  				.$TableauHeure[$p]['data']['nom']." "
		  				.$TableauHeure[$p]['data']['prenom']
		  				." Né(e) le ".date('d/m/Y',strtotime($TableauHeure[$p]['data']['ddn']))
		  				."(".$TableauHeure[$p]['data']['sex'].")</a>";		         		  				
		  		$lien .=$Html->InputSelect($ListeTypeResa,'color',$TableauHeure[$p]['data']['type'],30,"class='color' LOC_ID='".$TableauHeure[$p]['data']['entry_id']."' COLOR_VAL='".$tab_couleur[$num_couleur]."'");		
			}
			echo "<tr bgcolor='".$CurColeur."' align='left'  LOC_ID='".$TableauHeure[$p]['data']['entry_id']."'>
							<td> ".$TableauHeure[$p]['plage']."</td>			
							<td>".$lien."</td>
							<td>".$Aghate->GetDescComplementaire($TableauHeure[$p]['data']['description'])."</td>
						</tr>";				
