...
if ($is_posted == '1') {
                ...
                for ($c=0; $c<$num; $c++) {
                    ...
                    case 0:
                        ...
                        if (ereg ("^[a-zA-Z0-9_.]{1,20}$", $data[$c])) {
                            ...
                            $test = grr_sql_count(grr_sql_query("SELECT login FROM agt_utilisateurs WHERE login='$data[$c]'"));
                            if ($test!='0') {
                    ...
                    case 1:
                        ...
                        if (ereg ("^.{1,30}$", $data[$c])) {
                            ...
                            $test_nom_prenom = grr_sql_count(grr_sql_query("SELECT nom FROM agt_utilisateurs WHERE (nom='$test_nom' and prenom = '$test_prenom')"));
    ...
    for ($row=1; $row<$nb_row; $row++) {
        ...
        $test_login = grr_sql_count(grr_sql_query("SELECT login FROM agt_utilisateurs WHERE login='$reg_login[$row]'"));
        if ($test_login == 0) {
            $regdata = grr_sql_query("INSERT INTO agt_utilisateurs SET nom='".$reg_nom[$row]."',prenom='".$reg_prenom[$row]."',login='".$reg_login[$row]."',email='".$reg_email[$row]."',password='".protect_data_sql($reg_mdp[$row])."',statut='".$reg_type_user[$row]."',etat='".$reg_statut[$row]."',source='".$reg_type_auth[$row]."'");
        } else {
            $regdata = grr_sql_query("UPDATE agt_utilisateurs SET nom='".$reg_nom[$row]."',prenom='".$reg_prenom[$row]."',email='".$reg_email[$row]."',password='".protect_data_sql($reg_mdp[$row])."',statut='".$reg_type_user[$row]."',etat='".$reg_statut[$row]."',source='".$reg_type_auth[$row]."' WHERE login='".$reg_login[$row]."'");
        }
        if (!$regdata) {
